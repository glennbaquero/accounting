...
class CustomerInvoiceVoucherJournalFetchController extends Controller
{
    ...
    {
        $this->class = new CustomerInvoiceApprovalVoucher;
    }
    ...
    public function filterQuery($query)
    {
        ...
        if($this->request->filled('status')) {
            ...
            if($status == 'approved') {
                $query = $query->whereNotNull('approved_date')->where('customer_invoice_journal_number', $this->request->id);
            }
            if($status == 'rejected') {
                $query = $query->whereNotNull('rejected_by_journal')->where('customer_invoice_journal_number', $this->request->id);
            }
            if($status == 'pending') {
                $query = $query->whereNull('approved_by_journal')->whereNull('approved_date')->where('customer_invoice_journal_number', $this->request->id);
            }
        }
