...
class PromissoryNoteFetchController extends Controller
{
    ...
    public function formatData($items)
    {
        ...
        foreach($items as $item) {
            array_push($result, [
                ...
                'promissory_note_journal_number' => $item->promissory_note_journal_number,
                ...
                'totalDebit' => number_format($item->promissory_note_vouchers->sum('debit_amount'), 2, '.', ','),
                'totalCredit' => number_format($item->promissory_note_vouchers->sum('credit_amount'), 2, '.', ','),
                'totalBalance' => number_format($item->promissory_note_vouchers->sum('balance_journal'), 2, '.', ','),
                'total_vouchers' => $item->promissory_note_vouchers()->count(),
                'total_log_errors' => $item->promissory_note_vouchers()->where('log_message', 'like', '%Error%')->count(),
    ...
    public function fetchView($id = null)
    {
        ...
        return response()->json([
            ...
            'clients' => $clients,
            'vendor_bank_accounts' => VendorBankAccount::get(),
    ...
    protected function formatView($item)
    {
        ...
        $item->createPaymentScheduleUrl = route('payment-schedules.create-pn', ['null', $item->promissory_note_journal_number, 'pn']);
