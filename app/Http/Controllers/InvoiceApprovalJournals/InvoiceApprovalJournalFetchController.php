...
class InvoiceApprovalJournalFetchController extends Controller
{
    /**
    ...
    {
        $this->class = new InvoiceApprovalJournal;
    }
    ...
    public function formatData($items)
    {
        ...
        foreach($items as $item) {
            array_push($result, [
                ...
                'invoice_approval_journal_number' => $item->invoice_approval_journal_number,
                'invoice_approval_journal_number' => $item->invoice_approval_journal_number,
                'invoice_approval_journal_number' => $item->invoice_approval_journal_number,
                ...
                'log_date' => $item->log_date ? Carbon::parse($item->log_date)->format('m/d/Y g:i A') : '---',
                ...
                'totalDebit' => number_format($item->invoice_approval_journal_vouchers->sum('debit_amount'), 2, '.', ','),
                'totalCredit' => number_format($item->invoice_approval_journal_vouchers->sum('credit_amount'), 2, '.', ','),
                'totalBalance' => number_format($item->invoice_approval_journal_vouchers->sum('balance_journal'), 2, '.', ','),
                'total_vouchers' => $item->invoice_approval_journal_vouchers()->count(),
                'total_log_errors' => $item->invoice_approval_journal_vouchers()->where('log_message', 'like', '%Error%')->count(),
            ]);
        }
    ...
    protected function formatView($item)
    {
        ...
        $item->total_logs = $item->invoice_approval_journal_vouchers()->where('log_message', 'like', '%Error%')->count();
        $item->total_vouchers = $item->invoice_approval_journal_vouchers()->count();
