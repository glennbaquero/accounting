...
class InvoiceApprovalJournalVoucherFetchController extends Controller
{
    /**
    ...
    {
        $this->class = new InvoiceApprovalJournalVoucher;
    }
    ...
    public function filterQuery($query)
    {
        ...
        if($this->request->filled('id')) {
            $query = $query->where('invoice_approval_journal_number', $this->request->id);
        }

        if($this->request->filled('vendor_invoice_number')) {
            $query = $query->where('vendor_invoice_number', $this->request->vendor_invoice_number);
        }
        ...
        if($this->request->filled('status')) {
            ...
            if($status == 'approved') {
                $query = $query->whereNotNull('approved_date')->where('invoice_approval_journal_number', $this->request->id);
            }
            if($status == 'rejected') {
                $query = $query->whereNotNull('rejected_by_journal')->where('invoice_approval_journal_number', $this->request->id);
            }
            if($status == 'pending') {
                $query = $query->whereNull('approved_by_journal')->whereNull('approved_date')->where('invoice_approval_journal_number', $this->request->id);
            }
        }
    ...
    public function formatData($items)
    {
        ...
        foreach($items as $item) {
            ...
            array_push($result, [
                ...
                'invoice_voucher_number' => $item->invoice_voucher_number,
                ...
                'voucher_number' => $item->invoice_voucher_number,
            ]);
        }
