...
class CustomerPaymentJournalFetchController extends Controller
{
    /**
    ...
    {
        $this->class = new CustomerPaymentJournal;
    }
    ...
    public function formatData($items)
    {
        ...
        foreach($items as $item) {
            array_push($result, [
                ...
                'totalDebit' => number_format($item->customer_payment_journal_vouchers->sum('debit_amount'), 2, '.', ','),
                'totalCredit' => number_format($item->customer_payment_journal_vouchers->sum('credit_amount'), 2, '.', ','),
                'totalBalance' => number_format($item->customer_payment_journal_vouchers->sum('balance_journal'), 2, '.', ','),

                'total_vouchers' => $item->customer_payment_journal_vouchers()->count(),
                'total_log_errors' => $item->customer_payment_journal_vouchers()->where('log_message', 'like', '%Error%')->count(),
            ]);
        }
    ...
    public function fetchView($id = null)
    {
        ...
        $invoices = CustomerPayment::where('company_id', auth()->user()->company_id)->get();
        $payment_methods = CustomerPaymentMethod::all();
    ...
    protected function formatView($item)
    {
        ...
        $item->total_logs = $item->customer_payment_journal_vouchers()->where('log_message', 'like', '%Error%')->count();
        $item->total_vouchers = $item->customer_payment_journal_vouchers()->count();
