...
// use App\Models\Invoices\CustomerPayment;
// use App\Models\Invoices\CustomerPaymentLine;
...
class CustomerPaymentJournalVoucherFetchController extends Controller
{
    /**
    ...
    {
        $this->class = new CustomerPaymentJournalVoucher;
    }
    ...
    public function filterQuery($query)
    {
        ...
        if($this->request->filled('id')) {
            $query = $query->where('customer_payment_journal_number', $this->request->id);
        }
    ...
    public function formatData($items)
    {
        ...
        foreach($items as $item) {
            ...
            if($item->main_account && $item->main_account != '---') {
                $main_account = MainAccount::withTrashed()->find($item->main_account);
                // if(!$main_account){
                //     return;
                // }
                ...
                $offset_account = MainAccount::withTrashed()->find($item->offset_account);
                // if(!$main_account){
                //     return;
                // }
            ...
            array_push($result, [
                ...
                'posted_by' => $item->posted_by ? $item->posted->fullname : null,
                ...
                'received_date' => $item->received_date  ? Carbon::parse($item->received_date)->format('m/d/Y') : null,
