...
class BillOfExchangeFetchController extends Controller
{
    /**
    ...
    public function formatData($items)
    {
        ...
        foreach($items as $item) {
            array_push($result, [
                ...
                'bill_exchange_journal_number' => $item->bill_exchange_journal_number,
                ...
                'totalDebit' => number_format($item->bill_of_exchange_vouchers->sum('debit_amount'), 2, '.', ','),
                'totalCredit' => number_format($item->bill_of_exchange_vouchers->sum('credit_amount'), 2, '.', ','),
                'totalBalance' => number_format($item->bill_of_exchange_vouchers->sum('balance_journal'), 2, '.', ','),
                'total_vouchers' => $item->bill_of_exchange_vouchers()->count(),
                'total_log_errors' => $item->bill_of_exchange_vouchers()->where('log_message', 'like', '%Error%')->count(),
            ]);
        }
    ...
    public function fetchView($id = null)
    {
        ...
        return response()->json([
            ...
            'customer_bank_accounts' => CustomerBankAccount::get(),
            'client_bank_accounts' => ClientBankAccount::get(),
        ]);
    }
