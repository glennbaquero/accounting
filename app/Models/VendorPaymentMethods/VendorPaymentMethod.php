...
namespace App\Models\VendorPaymentMethods;
...
class VendorPaymentMethod extends Model
{
	...
	])
	{
	    ...
	    if (!$item) {
	    	...
	    	// $method_of_payment_id = 'vpm-' . date('Ymd') . '-' . str_pad($item->id, 4, '0', STR_PAD_LEFT);
	        $item->update([
	...
	public function renderShowUrl() {
        return route('vendor-payment-methods.show', $this->id);
    }

    public function renderArchiveUrl() {
        return route('vendor-payment-methods.archive', $this->id);
    }

    public function renderRestoreUrl() {
        return route('vendor-payment-methods.restore', $this->id);
    }
}
