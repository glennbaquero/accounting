...
class GeneralJournal extends Model
{
	...
	public function toSearchableArray() {
	    return [
	        'id' => $this->id,
	        'general_journal_number' => $this->general_journal_number,
	...
	public function general_journal_vouchers() {
		return $this->hasMany(GeneralJournalVoucher::class, 'general_journal_number', 'general_journal_number');
	}
	...
	{
		$vouchers = $this->general_journal_vouchers;
		$data = [];

		foreach ($vouchers as $line) {
			array_push($data, [
				'id' => $line->id,
				'general_journal_voucher_number' => $line->general_journal_voucher_number,
	...
	public function renderShowUrl() {
        return route('general-journal.show', $this->id);
    }

    public function renderArchiveUrl() {
        return route('general-journal.archive', $this->id);
    }

    public function renderRestoreUrl() {
        return route('general-journal.restore', $this->id);
    }
    
    public function renderCreateLineUrl() {
    	return route('general-journal.create', $this->id);
    }

    public function renderValidateUrl() {
    	return route('general-journal.validate', $this->id);
    }
    ...
    public function renderEditUrl() {
    	return route('general-journal.edit', $this->id);
    }
    ...
    // 	return route('general-journal.update', $this->id);
    // }
    ...
    // 	return number_format($this->general_journal_vouchers->sum('credit_amount'), 2, '.', ',');
    // }

    // public function getTotalDebitAttribute() {
    // 	return number_format($this->general_journal_vouchers->sum('debit_amount'), 2, '.', ',');
    // }

    // public function getTotalBalanceAttribute() {
    // 	return number_format($this->general_journal_vouchers->sum('balance_journal'), 2, '.', ',');
    // }
}
