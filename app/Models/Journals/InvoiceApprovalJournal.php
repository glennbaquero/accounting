...
class InvoiceApprovalJournal extends Model
{

	// protected $appends = [ 'is_selected', 'updateUrl', 'createLineUrl', 'archiveUrl', 'showUrl', 'restoreUrl', 'totalDebit', 'totalCredit', 'totalBalance', 'validateUrl' ];

	public function toSearchableArray() {
	    return [
			'id' => $this->id,
	        'invoice_approval_journal_number' => $this->invoice_approval_journal_number,
	...
	public function invoice_approval_journal_vouchers() 
	{
		return $this->hasMany(InvoiceApprovalJournalVoucher::class, 'invoice_approval_journal_number', 'invoice_approval_journal_number');
	}
	...
	$columns = [
		'invoice_approval_journal_number', 
		'invoice_journal_batch_number', 
		'journal_name_number', 
		'journal_name', 
		'description', 
		'journal_status', 
		...
		'detail_level', 
		'posting_layer', 
		'number_allocation_at_posting', 
		'delete_lines_after_posting', 
		'lines_limit', 
		'remittance_type', 
		'bank_account', 
		'protest_settlements', 
	...
	{
		$vouchers = $this->invoice_approval_journal_vouchers;
		$data = [];
	...
	public function renderShowUrl() {
        return route('po-invoice-approval-journals.show', $this->id);
    }

    public function renderArchiveUrl() {
        return route('po-invoice-approval-journals.archive', $this->id);
    }

    public function renderRestoreUrl() {
        return route('po-invoice-approval-journals.restore', $this->id);
    }
    
    public function renderCreateLineUrl() {
    	return route('po-invoice-approval-journals.create', $this->id);
    }

    public function renderValidateUrl() {
    	return route('po-invoice-approval-journals.validate', $this->id);
    }
    ...
    public function renderEditUrl() {
    	return route('po-invoice-approval-journals.edit', $this->id);
    }
    ...
    // 	return route('po-invoice-approval-journals.update', $this->id);
    // }
    ...
    // public function getCreateLineUrlAttribute() {
    // 	return route('po-invoice-approval-journals.create', $this->id);	
    // }
    ...
    // 	return number_format($this->invoice_approval_journal_vouchers->sum('credit_amount'), 2, '.', ',');
    // }

    // public function getTotalDebitAttribute() {
    // 	return number_format($this->invoice_approval_journal_vouchers->sum('debit_amount'), 2, '.', ',');
    // }

    // public function getTotalBalanceAttribute() {
    // 	return number_format($this->invoice_approval_journal_vouchers->sum('balance_journal'), 2, '.', ',');
    // }
}
