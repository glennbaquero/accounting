...
class CustomerPaymentJournal extends Model
{
	...
	public function customer_payment_journal_vouchers() 
	{
		return $this->hasMany(CustomerPaymentJournalVoucher::class, 'customer_payment_journal_number', 'customer_payment_journal_number');
	}
	...
	{
		$vouchers = $this->customer_payment_journal_vouchers;
		$data = [];
		...
		foreach ($vouchers as $line) {
			array_push($data, [
				...
				'customer_payment_journal_number' => $line->customer_payment_journal_number,
				...
				'posted_by' => $line->posted_by ? $line->posted->fullname : null,
	...
	public function renderShowUrl() {
        return route('customer-payment-journals.show', $this->id);
    }

    public function renderArchiveUrl() {
        return route('customer-payment-journals.archive', $this->id);
    }

    public function renderRestoreUrl() {
        return route('customer-payment-journals.restore', $this->id);
    }

    public function renderCreateLineUrl() {
    	return route('customer-payment-journals.create', $this->id);
    }
    
    public function renderValidateUrl() {
    	return route('customer-payment-journals.validate', $this->id);
    }
    ...
    public function renderEditUrl() {
    	return route('customer-payment-journals.edit', $this->id);
    }
    ...
    // 	return route('customer-payment-journals.update', $this->id);
    // }
    ...
    // 	return number_format($this->customer_payment_journal_vouchers->sum('credit_amount'), 2, '.', ',');
    // }

    // public function getTotalDebitAttribute() {
    // 	return number_format($this->customer_payment_journal_vouchers->sum('debit_amount'), 2, '.', ',');
    // }

    // public function getTotalBalanceAttribute() {
    // 	return number_format($this->customer_payment_journal_vouchers->sum('balance_journal'), 2, '.', ',');
    // }
}
