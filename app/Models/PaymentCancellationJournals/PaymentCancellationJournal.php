...
namespace App\Models\PaymentCancellationJournals;
...
class PaymentCancellationJournal extends Model
{
    /**
	...
	public function toSearchableArray() {
	    return [
	        'id' => $this->id,
	        'payment_cancellation_journal_number' => $this->payment_cancellation_journal_number,
	...
	])
	{
	    ...
	    if (!$item) {
	        ...
	        $payment_cancellation_journal_number = 'payment-cancellation-journal-' . date('Ymd') . '-' . str_pad($item->id, 4, '0', STR_PAD_LEFT);
	        $item->update([
	        	'created_by' => $request->user()->id,
	        	'updated_by' => $request->user()->id,
	        	'payment_cancellation_journal_number' => $payment_cancellation_journal_number,
	        ]);
	    } else {
	        $item->update($vars);
	        $item->update([
	...
	public function renderShowUrl() {
        return route('payment-cancellation-journals.show', $this->id);
    }

    public function renderArchiveUrl() {
        return route('payment-cancellation-journals.archive', $this->id);
    }

    public function renderRestoreUrl() {
        return route('payment-cancellation-journals.restore', $this->id);
    }
}
