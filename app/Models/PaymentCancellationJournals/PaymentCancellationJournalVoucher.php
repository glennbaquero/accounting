...
class PaymentCancellationJournalVoucher extends Model
{
	protected $casts = [
	...
	])
	{
	    ...
	    if (!$item) {
	        ...
	        // $reversal_id = 'reversal-id-' . date('Ymd') . '-' . str_pad($item->id, 4, '0', STR_PAD_LEFT);
	        $item->update([
	...
	public function renderUpdateUrl() {
        return route('payment-cancellation-journal-vouchers.update', $this->id);
    }

	public function renderShowUrl() {
        return route('payment-cancellation-journal-vouchers.fetch-item', $this->id);
    }

    public function renderArchiveUrl() {
        return route('payment-cancellation-journal-vouchers.archive', $this->id);
    }

    public function renderRestoreUrl() {
        return route('payment-cancellation-journal-vouchers.restore', $this->id);
    }
