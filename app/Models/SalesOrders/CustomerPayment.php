...
class CustomerPayment extends Model
{
	...
	public function toSearchableArray() {
	    return [
            'id' => $this->id,
            'customer_payment_number' => $this->customer_payment_number,
    ...
    public static function store($request, $item = null, $columns = ['customer_invoice_id', 
    ...
    public function generateCheckParent($request) {
    	...
    	$request->merge([
	    	...
	    	'customer_payment_id' => $this->customer_payment_number, 
    ...
    public function generateDepositParent($request) {
    	...
    	$request->merge([
	    	...
	    	'method_of_payment_vendor' => $this->method_of_payment_id, 
	    	'customer_payment_status' => $this->payment_status, 
	    	'customer_payment_id' => $this->customer_payment_number, 
    	]);
    ...
    public function createPaymentJournalEntry(
		CustomerPaymentJournal $journal_entry,
		...
		$payment_line_type = 'approved_payment') 
	{
        try {
			...
			$offset_posting_profile = $this->customer_posting_profile->posting_lines->first();
			$summary_account_posting_profile = $this->customer_posting_profile->posting_lines->first();
			$offset = $offset_posting_profile->offset_account;
			$summary_account = $summary_account_posting_profile->summary_account;

			$lines = $this->customer_payment_lines()
						->where($payment_line_type, true)
						->get();
			foreach ($lines as $line) {
				$count = CustomerPaymentJournalVoucher::count();
				...
				$last_added_voucher = $journal_entry->customer_payment_journal_vouchers()->latest()->first();

				if($last_added_voucher->main_account != '---') {
					$voucher = $journal_entry->customer_payment_journal_vouchers()->create([
						'voucher_number' => $order_number,
						'customer_payment_journal_number' => $journal_entry->customer_payment_journal_number,
						...
						'customer_name' => $this->customer ? $this->customer->fullname : '---',
						'payment_id' => $this->customer_payment_number ?? $payment_id,
    ...
    public function customer_payment_lines() {
    ...
    public function customer() {
        return $this->belongsTo(Customer::class, 'customer_account_id', 'id')->withTrashed();
    }
    ...
    public function customer_invoice() {
        return $this->belongsTo(CustomerInvoice::class)->withTrashed();
    }

    public function customer_posting_profile() {
    	return $this->belongsTo(CustomerPostingProfileHeader::class, 'posting_profile')->withTrashed();
    }

    public function renderShowUrl() {
        return route('customer-payments.show', $this->id);
    }

    public function renderArchiveUrl() {
        return route('customer-payments.archive', $this->id);
    }

    public function renderRestoreUrl() {
        return route('customer-payments.restore', $this->id);
    }

    public function renderApprovalUrl() {
        return route('customer-payments.approved', $this->id);
    }

    public function renderPostUrl() {
        return route('customer-payments.posted', $this->id);
    }
}