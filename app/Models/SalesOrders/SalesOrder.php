...
class SalesOrder extends Model
{
	...
	public function sales_order_lines() {
		return $this->hasMany(SalesOrderLine::class, 'sales_order_number', 'sales_order_number');
	}
	...
	public function customer_invoice() {
		return $this->hasOne(CustomerInvoice::class, 'sales_order_number', 'sales_order_number');
	}

	public function customer_invoices() {
		return $this->hasMany(CustomerInvoice::class, 'sales_order_number', 'sales_order_number');
	}
	...
	public function credits() {
		return $this->hasMany(LetterCreditSales::class);
	}
	 ...
	 * @Setters
	 */
	public static function store($request, $item = null, $columns = ['sales_order_number', 'customer_account', 'invoice_account', 'sales_order_date', 'delivery_date', 'due_date', 'approval_status_date', 'confirmed_date', 'accounting_date', 'customer_name', 'customer_address', 'customer_contact_id', 'confirmed_by', 'approver', 'ordered_by', 'created_by', 'updated_by', 'cost_center_id', 'department_id', 'expense_purpose_id', 'posting_profile', 'accounting_distribution', 'method_of_payment', 'terms_of_payment', 'payment_specification', 'sales_tax_group', 'tax_exempt_number', 'sales_type', 'sales_order_status', 'document_status', 'approval_status', 'settlement_type', 'prices_include_sales_tax', 'delivery_terms', 'mode_of_delivery', 'charges_group', 'cash_discount', 'line_discount_group', 'multiline_disc_group', 'total_discount_group', 'update_quantity_type', 'total_data_quantity', 'total_data_volume', 'total_line_discount', 'subtotal_amount', 'total_discount', 'total_charges', 'total_sales_tax', 'total_round_off', 'total_amount', 'total_cash_discount', 'sales_orders', 'sold_by', 'client_id', 'total_sales_vat_exclusive', 'less_discount', 'add_charge', 'add_vat', 'total_sales_vat_inclusive', 'less_withholding_tax', 'amount_due', 'vattable_sales', 'vat_exempt_sale', 'zero_rated_sales', 'vat_amount', 'total_amount_due', 'cash_amount', 'check_amount', 'deposit_amount', 'other_amount', 'total_amount_received', 'outstanding', 'tax_posting_id', 'payment_schedule_id', 'charge_id', 'discount_id', 'bank_document_type'])
	{
	...
	public function renderShowUrl() {
        return route('sales-orders.show', $this->id);
    }

    public function renderArchiveUrl() {
        return route('sales-orders.archive', $this->id);
    }

    public function renderRestoreUrl() {
        return route('sales-orders.restore', $this->id);
    }

    public function renderConfirmationUrl() {
        return route('sales-orders.confirmation', $this->id);
    }

    public function renderCustomerInvoiceUrl() {
        return route('customer-invoices.create', $this->sales_order_number);
    }

    public function renderCOP() {
    	$amount = 0.00;
    	$lines = $this->sales_order_lines;
