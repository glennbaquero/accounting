...
<div class="content-wrapper">
    ...
    <section class="content-header">
        <div class="row mb-2">
            <div class="col-sm-6">
                <h1>Update Purchase Delivery Receipt <small>({{ $item->purchase_delivery_receipt_number }})</small></h1>
            </div>
            <div class="col-sm-6">
                <ol class="breadcrumb float-sm-right">
                    <li class="breadcrumb-item"><a href="{{ route('purchase-delivery-receipts.index') }}">Purchase Delivery Receipts</a></li>
    ...
    <section class="content">
        <purchase-delivery-receipt-view
            submit-url="{{ route('purchase-delivery-receipts.update', $item->id) }}"
            fetch-url="{{ route('purchase-delivery-receipts.fetch-item', [ $item->purchase_order_number ?? 'null', $item->id ]) }}"
            ...
            invoice-approval-url="{{ route('po-invoice-approval-journals.fetch-vouchers', ['purchase_delivery_receipt_number' => $item->purchase_delivery_receipt_number]) }}"
            vendor-payment-url="{{ route('vendor-payment-journals.fetch-vouchers', ['purchase_delivery_receipt_number' => $item->purchase_delivery_receipt_number]) }}"
            general-journal-url="{{ route('general-journal.fetch-vouchers', ['purchase_delivery_receipt_number' => $item->purchase_delivery_receipt_number]) }}"
            print-url="{{ route('purchase-delivery-receipts.print',  ['id' => $item->id ] ) }}"
