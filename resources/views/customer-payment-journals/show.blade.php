...
<div class="content-wrapper">
    ...
    <section class="content-header">
        <div class="row mb-2">
            <div class="col-sm-6">
                <h1>Customer Payment Journal Voucher <small>({{ $item->customer_payment_journal_number }})</small></h1>
            </div>
            <div class="col-sm-6">
    ...
    <section class="content">
        {{-- <subsidiary-view
            ...
            invoice-approval-url="{{ route('customer-payment-journals.fetch-vouchers') }}"
        ></subsidiary-view> --}}
            ...
            <div class="card">
                <div class="card-header p-2">
                    <ul class="nav nav-pills">
                        <li class="nav-item"><a class="nav-link active" href="#customer-payment-journal" data-toggle="tab">Customer Payment Journal</a></li>
                        <li class="nav-item"><a class="nav-link" href="#invoice-approval-journal" data-toggle="tab">Invoice Approval Journal</a></li>
                    </ul>
                </div>
                <div class="card-body">
                    <div class="tab-content">
                        <div class="tab-pane show active" id="customer-payment-journal">
                            <customer-payment-journal-view
                                submit-url="{{ route('customer-payment-journals.update', $item->id) }}"
                                voucher-submit-url="{{ route('customer-payment-journals.voucher-create', $item->id) }}"
                                fetch-voucher-url="{{ route('customer-payment-journals.fetch-vouchers', ['id' => $item->customer_payment_journal_number]) }}"
                                fetch-url="{{ route('customer-payment-journals.fetch-item', $item->id) }}"
                                status-update-url="{{ route('customer-payment-journals.voucher-status-update') }}"
                                journal-validate-url="{{ route('customer-payment-journals.validate', $item->id) }}"
                                voucher-validate-url="{{ route('customer-payment-journals.validate-voucher') }}"
                                post-url="{{ route('customer-payment-journals.post', $item->id) }}"
                                :journal-item="{{ $item }}"
                            ></customer-payment-journal-view>
                        </div>
