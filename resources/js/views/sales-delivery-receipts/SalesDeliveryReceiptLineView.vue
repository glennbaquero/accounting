		...
		<div class="row">
			<div class="col-md-12">
				...
				<data-table 
					...
					ref="data-table"
					:headers="headers" 
					:items="sales_delivery_receipt_lines">
						<template v-slot:body="{ items }">
						<tr v-for="(table_item, key) in items" v-bind:key="key">
							<td>{{ table_item.sales_delivery_receipt_line_number }}</td>
							...
							<td>
								<template v-if="table_item.id">
									<template v-if="showConfirmButton">
										...
										title="Approve Line"
										alt-title=""
										:message="'Are you sure you want to approved this line ' + table_item.sales_delivery_receipt_line_number + '?'"
										:alt-message="'Are you sure you want to reject this line ' + table_item.sales_delivery_receipt_line_number + '?'"
										@load="load"
						...
						<div class="row">
							<div class="col-md-2">
								<div class="form-group">
									...
									<input readonly class="form-control mb-2" v-model="item.sales_delivery_receipt_line_number">
...
<script>
	...
	export default {
        ...
        computed: {
            ...
            totalQuantity() {
            	var sales_delivery_receipt_lines = this.sales_delivery_receipt_lines;

            	var total = 0;

            	_.each(sales_delivery_receipt_lines, (line) => {
		...
		watch : {
			...
			'ci.sales_order_number'(value) {
				this.sales_delivery_receipt_lines.forEach(line => line.sales_order_number = value);
				this.changeKey();
			},
			...
			'ci.customer_account'(customer_account) {
				...
				this.sales_delivery_receipt_lines.forEach(line => {
			...
			'lines'(value) {
				if (value) {
					if (! this.sales_delivery_receipt_lines) {
						this.sales_delivery_receipt_lines = [];
					}
					value.forEach(line => {
						const index = this.sales_delivery_receipt_lines.findIndex(cline => cline.sales_delivery_receipt_line_number == line.sales_delivery_receipt_line_number);
						if (index != -1) {
							this.sales_delivery_receipt_lines[index] = line;
						} else {
							this.sales_delivery_receipt_lines.push(line);
						}
						this.changeKey();
					});

					this.$emit('newLines',this.sales_delivery_receipt_lines);
				}
			},
		...
		methods: {
			...
			addLine() {
				...
				if(this.validateRequiredFields()) {
					...
					this.sales_delivery_receipt_lines.push(this.item);
			...
			removeLine(key) {
				this.sales_delivery_receipt_lines.splice(key, 1);
				this.$emit('newLines',this.sales_delivery_receipt_lines);
			},

			generateLineCode(value, prefix) {
				if(value == 'create') {
					this.item.sales_delivery_receipt_line_number = this.generateLineCodeV2(prefix);
				}
			},
			...
			validateRequiredFields() {
				...
				if(!this.item.sales_category || unit_price < 1 ||
					!this.item.cost_center_id || !this.item.department_id || 
					!this.item.expense_purpose_id) {
					swal.fire({
					icon: 'error',
			...
			editLine(key) {
				this.item = Object.assign({}, this.sales_delivery_receipt_lines[key]);
			...
			saveLine() {
				if (this.validateRequiredFields()) {
					$('#so-lines-modal').modal('hide');
					this.sales_delivery_receipt_lines[this.editing_key] = this.item;
