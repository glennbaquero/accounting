		...
		<form-request :submit-url="submitUrl" @load="load" @success="fetch" confirm-dialog sync-on-success :params="item">
			...
			<card>
				<template v-slot:header>
					Methods of Payment - Customer Information
						...
						<div class="row">
							<div class="col-md-9">
								<ul class="nav nav-pills">
									<li class="nav-item"><a class="nav-link active" href="#methods" data-toggle="tab">Methods of Payment - Customer</a></li>
								</ul>
							</div>
						</div>
				    </div>
...
<script>
	...
	export default {
		...
		computed: {
			mainAccount() {
				let item = this.mainAccounts.find((account) => {
		...
		watch: {
			'item.client_id'(value) {
				this.filteredMainAccounts = this.mainAccounts.filter((data) => {
