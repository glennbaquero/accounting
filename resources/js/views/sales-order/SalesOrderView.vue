				        		...
				        		<div class="row">
				        			<div class="col-md-4">
			        		    		<div class="form-group">
			        		    			...
			        		    			<input type="text" name="sales_order_number" v-model="item.sales_order_number" class="form-control mb-2" readonly>
		        				...
		        				<div class="row">
									<div class="col-md-6">
										<div class="form-group">
											...
											<v-select v-model="item.method_of_payment" :options="payment_methods" label="method_of_payment" placeholder="Select Payment Method" :reduce="item => item.id" class="mb-2"></v-select>							    		
...
<script>
	...
	export default {
		...
		watch: {
			line_total_amount(val) {
				setTimeout(() => {
					this.item.total_sales_vat_exclusive = val
				}, 500)
			},
		...
		mounted() {
			...
			if(!this.item.sales_order_number) {
				this.generateID();
			}
		...
		methods: {
			fetchSuccess(data) {
				...
				this.bills_of_exchanges = data.bills_of_exchanges ?? this.bills_of_exchanges;
				this.credits = data.credits ?? this.credits;
				this.guarantees = data.guarantees ?? this.guarantees;
			...
			getActiveCode() {
				...
				if(id) {
					...
					if(client.code) {	
						this.item.sales_order_number = client.code;
					}else {
						this.generateID();
					}	
				}
			...
			generateID() {
				...
				this.item.sales_order_number = date.getDate().toString() + date.getMonth().toString() + date.getFullYear().toString() +'-'+ time.toString();
				this.item.sales_order_number += "-" + Math.random().toString(36).substring(2, 6);
			}
		}
	}
</script>