			...
			<card>
				<template v-slot:header>
					...
					<div class="float-right">
						...
						<button type="button" class="btn btn-success btn-sm" @click="confirmedPO" :disabled="disableConfirmButton">Confirm PO</button>
				...
				<div v-if="item.id ? true : false" class="row mb-3">
					<div class="col-md-12">
						<a :href='printUrl' type='button' class="btn btn-success" target="_blank">Print Purchase Order</a>
					</div>
				</div>
				<div class="card">
						...
						<div class="row">
							<div class="col-md-9">
								<ul class="nav nav-pills">
									<li class="nav-item"><a class="nav-link active" href="#purchase_order" data-toggle="tab">Purchase Order</a></li>
									<li class="nav-item"><a class="nav-link" href="#financial" data-toggle="tab">Financial Dimensions</a></li>
									<li class="nav-item"><a class="nav-link" href="#delivery" data-toggle="tab">Payment and Delivery</a></li>
									<!-- <li class="nav-item"><a class="nav-link" href="#tax" data-toggle="tab">VAT</a></li> -->
									<li class="nav-item"><a class="nav-link" href="#purchase_order_lines" data-toggle="tab">Purchase Order Lines</a></li>
				        		...
				        		<div class="row">
				        			<div class="col-md-4">
			        		    		<div class="form-group">
											<h4 class="mb-2"><i class="fas fa-tags"></i> Purchase Order</h4><hr>
											
											<label>Purchase Order Number</label>
											<div class="input-group mb-2">
												<input type="text" name="purchase_order_number" v-model="item.purchase_order_number" class="form-control mb-2" readonly>
											</div>
											<label>Purchase Order Return No.</label>
											<div class="input-group mb-2">
												<input type="text" name="purchase_order_return_id" v-model="item.purchase_order_return_id" class="form-control mb-2" readonly>
											</div>
											<label>Purchase Order Date <b class="text-danger">*</b></label>
											<div class="input-group mb-2">
				        	...
				        	<div class="tab-pane" id="financial">
		        				<div class="row">
		        		    		<div class="form-group col-sm-6">
		        		    			...
		        		    			<v-select class="mb-2" v-model="item.posting_profile_id" :reduce="item => item.id" label="posting_profile" :options="posting_profiles"></v-select>
		        				...
		        				<div class="row">
									<div class="col-md-6">
										<div class="form-group">
			        		                ...
			        		                <select class="form-control" v-model="item.return_action" name="return_action">
			        		                	<option value="Exact Payment">Exact Payment</option>
			        		                	<option value="Under Payment">Under Payment</option>
			        		                	<option value="Over Payment">Over Payment</option>
			        		                </select>
			        		                <label>Payment Level</label>
			        		                <select class="form-control" v-model="item.payment_level" name="payment_level">
			        		                	<option value="Exact Delivery">Exact Delivery</option>
			        		                	<option value="Under Delivery">Under Delivery</option>
			        		                	<option value="Over Delivery">Over Delivery</option>
			        		                </select>
			        		                <input class="mt-2 mr-2" name="prepayment" :checked="checked(item.prepayment)" type="checkbox"> <label>Prepayment</label>
								...
								<div class="row mt-4">
                                    ...
                                    <div class="col-4">
                                        ...
                                        <div class="form-group col-12">
                                        	...
                                        	<select class="form-control" name="bank_document_type" v-model="item.bank_document_type">
	...
	export default {
        ...
        computed: {
            ...
            filtered_charges_on_lines() {
            	return _.filter(this.charges_on_lines, (charge) => { return moment(this.sales_order_date).isBetween(charge.valid_from, charge.valid_to) });
            },

            filtered_discount_on_lines() {
            	return _.filter(this.discount_on_lines, (discount) => { return moment(this.sales_order_date).isBetween(discount.valid_from, discount.valid_to) });
            },

            filtered_charges_on_header() {
            	return _.filter(this.charges_on_header, (charge) => { return moment(this.sales_order_date).isBetween(charge.valid_from, charge.valid_to) });
            },

            filtered_discount_on_header() {
            	return _.filter(this.discount_on_header, (discount) => { return moment(this.sales_order_date).isBetween(discount.valid_from, discount.valid_to) });
            },
		...
		mounted() {
			...
			if(!this.item.purchase_order_number) {
		...
		methods: {
			...
			generateCode(value, prefix) {
				if(value == 'create') {
					...
					this.item.purchase_order_number = prefix + '-' + date.getFullYear().toString() + ("0" + (date.getMonth() + 1)).slice(-2) + date.getDate().toString()   +'-'+ time.toString();
				}
			},
			...
			getActiveCode() {
				...
				if(id) {
					...
					if(client.code) {	
						this.item.purchase_order_number = client.code;
					}else {
						this.generateCode('create','PO');
					}	
				}else {
