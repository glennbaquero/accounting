<template>
	<div>
		<!-- <div class="row">
			<div class="col-md-12 text-right">
				<button type="button" class="btn btn-success w-10" data-toggle="modal" @click="initDefaultValue" data-target="#po-lines-modal" data-backdrop="static"><i class="fas fa-plus"></i> Create Purchase Return Line</button>
			</div>
		...
		<div class="row">
			...
			<div class="form-group col-md-4">
				...
				<input name="total_charges"  type="text" :value="totalCharges | currency" readonly class="form-control">
			...
			<div class="col-md-12">
				...
				>
						...
						<tr v-for="(table_item, key) in items">
							<td>{{ table_item.purchase_order_line_number }}</td>
							<td>{{ table_item.product.item_number ? table_item.product.item_number : 20210292 }}</td>
	...
	<div class="modal fade"  id="po-lines-modal" tabindex="-1" role="dialog" aria-labelledby="purchase_order_line_modal_label" aria-hidden="true">
			<div class="modal-dialog modal-lg modal-dialog-centered" role="document">
				<div class="modal-content">
					<div class="modal-header">
						<h5 class="modal-title"  id="purchase_order_line_modal_label">Purchase Order Return Line</h5>
						...
						<div class="row">
							<div class="col-md-2">
								<div class="form-group">
									...
									<input readonly class="form-control mb-2" v-model="po.purchase_order_number">
									<label>Purchase Order Return Line Number</label>
									<input readonly class="form-control mb-2" v-model="item.purchase_order_line_number">
									<label>Vendor Account</label>
									<input readonly v-model="po.vendor_account" class="form-control mb-2">
									<label>Invoice Account</label>
									<input readonly v-model="po.invoice_account" class="form-control mb-2">
									<label>Vendor Name</label>
									<input readonly v-model="po.vendor_name" type="text" class="form-control mb-2">
							...
							<div class="col-md-2">
								<div class="form-group">
									...
									<input readonly v-model="po.delivery_date" class="form-control mb-2">
									<label>Delivery Type</label>
									<input readonly v-model="po.mode_of_delivery_type" class="form-control mb-2"> -->
							...
							<div class="col-md-2">
								<div class="form-group">
									...
									<!-- <input class="form-control mb-2" v-model="item.service_task" :readonly="item.order_line_type === 'product'"> -->
							...
							<div class="col-md-2">
								<div class="form-group">
									...
									<input readonly :value="po.formatted_created_at" class="form-control mb-2">
									<label>Updated By</label>
									<input readonly :value="updated_by"  class="form-control mb-2">
									<label>Updated On</label>
									<input readonly :value="po.formatted_updated_at" class="form-control mb-2">
...
<script>
	...
	import PurchaseOrderLineMixin from './purchase-order-line.js';
	...
	export default {
		...
		mixins: [ CrudMixin, PurchaseOrderLineMixin ],
		...
		mounted() {
			...
			this.item.procurement_category = this.procurement_categories[0],
			this.initDefaultValue();
			this.generateLineCode('create','POL');
		...
		watch : {
			...
			'lines'(value) {
				this.purchase_order_lines = value;
			},
		...
		methods: {
			...
			addLine() {
				if(this.validateRequiredFields()) {
					...
					this.purchase_order_lines.push(this.item);
					this.$emit('newLines',this.purchase_order_lines);
					this.initDefaultValue();
			...
			editLine(key) {
				...
				let item = this.purchase_order_lines[key];
				this.item = {...item};
			...
			saveLine() {
				if(this.validateRequiredFields()) {
					this.purchase_order_lines[this.editing_key] = {...this.item};
			...
			removeLine(key) {
				swal.fire({
					...
					if (result.isConfirmed) {
						 swal.fire(
							'Deleted!',
							'Purchase line has been deleted.',
							'success'
						)
						this.purchase_order_lines.splice(key, 1);
						this.$emit('newLines',this.purchase_order_lines);
					}
				})
			},
			...
			generateLineCode(value, prefix) {
				if(value == 'create') {
					...
					this.item.purchase_order_line_number = prefix + '-' + date.getFullYear().toString() + ("0" + (date.getMonth() + 1)).slice(-2) + date.getDate().toString()   +'-'+ time.toString();
				}
			},
			...
			validateRequiredFields() {
				// || !this.item.variant_id || !this.item.product_id || !this.item.quantity || this.item.quantity <= 0 )||
				if(!this.item.line_status || !this.item.procurement_category || !this.item.department_id || !this.item.expense_purpose_id || !this.item.cost_center_id)  
				{
					swal.fire({
					icon: 'error',
			...
			initDefaultValue() {
				...
				this.item = {
					...
					procurement_category: this.procurement_categories[0],
					variant : {},
					product : {},
					department_id : this.po.department,
					expense_purpose_id : this.po.expense_purpose,
					cost_center_id : this.po.cost_center,
				};
