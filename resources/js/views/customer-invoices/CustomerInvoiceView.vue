			...
			<card>
				<template v-slot:header>
					...
					<div class="float-right">
						...
						<button type="button" class="btn btn-success" :disabled="!item.id" @click="generateSalesDeliveryReceipt">Generate Sales Delivery Receipt</button>
				...
				<div class="row mb-3">
					<div class="col-md-12">
						<a :href='printUrl' type='button'  class="btn btn-success" target="_blank">Print Customer Invoice</a>
					</div>
				</div>
						...
						<div class="row">
							<div class="col-md-9">
								<ul class="nav nav-pills">
									<li class="nav-item"><a class="nav-link active" href="#customer-invoice" data-toggle="tab">Customer invoice</a></li>
									...
									<li class="nav-item"><a class="nav-link" href="#customer-invoice-lines" data-toggle="tab">Customer Invoice Lines</a></li>
				        	...
				        	<div class="tab-pane show active" id="customer-invoice">
								<div class="row">				
									<div class="col-md-3">
                                        ...
                                        <div class="form-group">
                                            <label for="customer_invoice_id">Sales Order Number</label>
											<v-select class="mb-2" 
												v-model="item.sales_order_number" 
												:options="sales_orders"
												:reduce="item => item.sales_order_number"
												label="sales_order_number"></v-select>
                                        </div>
                                        <div class="form-group">
...
<script>
	...
	import InvoiceCustomerLines from './CustomerInvoiceLineView.vue';
	...
	export default {
			...
			'item.sales_order_number'(value) {
				if (value) {
					if (this.original_sales_order_number != value) {
						...
						if (sales_order) {
							let customer_invoice_number = this.item.customer_invoice_number;
							this.item = Object.assign(this.item, sales_order);
							this.item.customer_invoice_number = customer_invoice_number;
							...
							this.item.sales_order_lines.forEach((line, index) => {
								...
								line.customer_invoice_number = this.item.invoice_account;
								line.customer_invoice_number = this.item.customer_invoice_number;
		...
		methods: {
			fetchSuccess(data) {
				...
				if(!this.item.customer_invoice_number) {
					this.generateID();
				}
			...
			generateSalesDeliveryReceipt() {
				...
				swal.fire({
				  ...
				  if (result.value) {
				    window.location.href = $this.item.generateSalesDeliveryReceiptUrl;
				  }
				})
			},
			...
			generateID() {
				...
				this.item.customer_invoice_number = date.getDate().toString() + date.getMonth().toString() + date.getFullYear().toString() +'-'+ time.toString();
				this.item.customer_invoice_number += "-" + Math.random().toString(36).substring(2, 6);
			},
			...
			getActiveCode() {
				...
				if(id) {
					...
					if(client.code) {	
						this.item.customer_invoice_number = client.code;
					}else {
						this.generateID();
					}	
				}else {
