			...
			<div class="col-md-12">	
				...
				>
						...
						<tr v-for="(table_item, key) in items" v-bind:key="key">
							<td>{{ table_item.vendor_invoice_line_number }}</td>
		...
		watch : {
			...
			'lines'(value) {
				if (value) {
					...
					value.forEach(line => {
						const index = this.vendor_invoice_lines.findIndex(vline => vline.vendor_invoice_line_number == line.vendor_invoice_line_number);
						if (index != -1) {
		...
		methods: {
			...
			generateLineCode(value, prefix) {
				if(value == 'create') {
					...
					this.item.vendor_invoice_line_number = prefix + '-' + date.getFullYear().toString() + ("0" + (date.getMonth() + 1)).slice(-2) + date.getDate().toString()   +'-'+ time.toString();
				}
			},
