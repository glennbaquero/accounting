	...
	<div>
		<form-request :submit-url="submitUrl" @load="load" @success="fetch" confirm-dialog sync-on-success :params="item">
			<card>
				...
				<div class="row mb-3">
					<div class="col-md-12">
						<a :href='printUrl' type='button'  class="btn btn-success" target="_blank">Print Vendor Invoice</a>
					</div>
				</div>
						...
						<div class="row">
							<div class="col-md-9">
								<ul class="nav nav-pills">
									<li class="nav-item"><a class="nav-link active" href="#vendor-invoice" data-toggle="tab">Vendor Invoice</a></li>
									...
									<li class="nav-item"><a class="nav-link" href="#vendor-invoice-lines" data-toggle="tab">Vendor Invoice Lines</a></li>
				        	...
				        	<div class="tab-pane show active" id="vendor-invoice">
								<div class="row">	
									<div class="col-md-3">
										...
										<input readonly class="form-control mb-2" v-model="item.vendor_invoice_number">
									    <!-- <div class="form-group">
...
<script>
	...
	import InvoiceVendorLines from './VendorInvoiceLineView.vue';
	...
	export default {
				...
				if (value) {
					if (this.original_purchase_order_number != value) {
						...
						if (purchase_order) {
							...
							let vendor_invoice_number = this.item.vendor_invoice_number;
							this.item = Object.assign(this.item, purchase_order);
							this.item.vendor_invoice_number = vendor_invoice_number;
							...
							this.item.purchase_order_lines.forEach((line, index) => {
								...
								line.vendor_invoice_number = this.item.invoice_account;
								line.vendor_invoice_number = this.item.vendor_invoice_number;
		...
		methods: {
			fetchSuccess(data) {
				...
				if(!this.item.vendor_invoice_number) {
					this.generateID();
			...
			generatePurchaseDeliveryReceipt() {
				...
				swal.fire({
				  title: 'Are you sure?',
				  text: 'Are you sure you want to generate purchase delivery receipt?',
				  icon: 'warning',
			...
			generateID() {
				...
				this.item.vendor_invoice_number =  'VI' + '-' + date.getFullYear().toString() + ("0" + (date.getMonth() + 1)).slice(-2) + date.getDate().toString()   +'-'+ time.toString();
			},
			...
			getActiveCode() {
				...
				if(id) {
					...
					if(client.code) {	
						this.item.vendor_invoice_number = client.code;
					}else {
						this.generateID();
					}	
				}
