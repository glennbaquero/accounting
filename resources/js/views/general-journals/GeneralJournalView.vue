	...
	<div>
		<form-request :submit-url="submitUrl" @load="load" @success="fetchData" confirm-dialog sync-on-success>
			<card>
				<template v-slot:header>
					...
					<div class="float-right">
						...
						:disabled="loading" 
						:action-url="reversalUrl"
						title="Reverse Journal"
						:message="'Are you sure you want to reverse this journal ?'" 
						class="btn-secondary">Generate Reversal
						</action-button>
					...
					<div class="row">
						...
						<div class="col-md-3">
							<div class="row">
								<div class="col-md-12">
									<div class="float-right">
										<button type="button" class="btn btn-success" @click="generateAccrual" :disabled="(status == 'pending' || status == 'rejected') || !hasVoucherSelected">Generate Accrual</button>
					...
					<div class="tab-content">
									...
									<div class="row">
										<div class="col-sm-12">
											<data-table 
												...
												:filters="filters"
												:fetch-url="fetchVoucherPendingUrl"
												order-by="id"
												order-asc
												@load="load"
												noAction
											>
												<template v-slot:body="{ items }">
													<tr v-for="$item in items" @click="selectedLine($item)" :class="$item.selected ? 'selected-table' : ''">
														...
														<td>{{ $item.general_journal_voucher_number }}</td>
						...
						<div class="tab-pane" id="journal-header">
							<journal-header
								...
								invoice-header-title="General Journal Number"
			...
			<new-voucher
				...
				voucher-identification="general_journal_voucher_number"
				journal-identification="general_journal_number"
				:isVendor="false"
				:main_accounts="main_accounts"
				:clients="clients"
				:client-id="journalItem.client_id"
				:invoices="invoices"
			></new-voucher>			
		</form-request>
	...
	export default {
        ...
        computed: {
            ...
            voucher_fields() {
            	let array = [
            	    ...
            	    { label: 'Voucher', value: this.selected.general_journal_voucher_number, editable: false, model: 'general_journal_voucher_number' },
            ...
            payment_fields() {
            	...
            	let array = [
            	    ...
            	    { label: 'Method of payment', value: this.selected.method_of_payment, editable: true, model: 'method_of_payment', isDropdownSelection: true, selections: this.payment_methods, opt_value: 'id', display: 'name' },
            	    ...
            	    { label: 'Terms of payment', value: this.selected.terms_of_payment, editable: true, model: 'terms_of_payment', isDropdownSelection: true, selections: this.terms_of_payments, opt_value: 'terms_of_payment', display: 'terms_of_payment' },
		...
		methods: {
			fetchSuccess(data) {
				...
				this.checker('General Journal', this.journalItem.general_journal_number)
			...
			generateAccrual() {
				console.log('start accrual generation')
	    		...
	    		axios.post(this.generateAccrualUrl, payloads)
	    		.then(response => {
	                this.$loading.show(false);
	                this.fetch();
	                this.selected = {};
					this.$refs['data-table'].selected = false;
	                this.parseSuccess(response.data.message, 'Accrual Generation')
	    		}).catch(error => {
	                this.$loading.show(false);
	                this.fetch();
	                this.selected = {};
